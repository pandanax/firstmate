...
- **Explicit project modes** - each project ships via `no-mistakes`, `direct-PR`, or `local-only`, with independent `merge:auto|manual` authority and an optional `+yolo` autonomy flag.
...
```sh
gh auth login
git clone https://github.com/pandanax/firstmate
cd firstmate
```
