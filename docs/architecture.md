...
`data/projects.md` records each project's delivery mode, `merge:auto|manual` authority, and optional `+yolo` autonomy flag.
`no-mistakes` projects run the full validation pipeline, `direct-PR` projects open PRs without that pipeline, and `local-only` projects stay local until firstmate performs an approved fast-forward merge.
The `bin/fm-project-mode.sh` header owns the registry syntax and resolution mechanics; `AGENTS.md` section 7 owns merge-authority behavior and safety.
